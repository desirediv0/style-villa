    ...
    if (error) {
        return (
            <div className="bg-red-50 border border-red-200 text-red-700 px-6 py-8 rounded-2xl text-center">
                ...
                <Button variant="outline" className="rounded-xl" onClick={() => router.push("/account/orders")}>
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
                <div>
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <h1 className="text-2xl md:text-3xl font-semibold">Order #{order.orderNumber}</h1>
                    </div>
                ...
                <div className="mt-4 md:mt-0 flex items-center gap-3">
                    ...
                    {order.status !== "CANCELLED" && order.status !== "DELIVERED" && order.status !== "SHIPPED" && (
                        <Button variant="destructive" size="sm" className="rounded-xl" onClick={handleCancelOrder} disabled={cancelling}>
                            {cancelling ? (
                                            ...
                                            {item.variant?.options && item.variant.options.length > 0 && (
                                                <div className="flex flex-wrap gap-2 mt-1">
                                                    {item.variant.options.map((opt, idx) => (
                                                        <span key={idx} className="text-xs text-gray-500 bg-gray-100 px-2 py-0.5 rounded-lg">
                                                            {opt.name}: {opt.value}
                                                        </span>
                                                    ))}
                                                </div>
